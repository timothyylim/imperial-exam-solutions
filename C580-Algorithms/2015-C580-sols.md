...
It seems that, the more bits there are, the more expensive it becomes (on average) to increment by one but the average cost given k seems to asymptotically approach a limit (sum from 0 to k of 2^-k = 2^0 + 2^-1 + 2^-2...). Not sure how that implies O(N) though...
