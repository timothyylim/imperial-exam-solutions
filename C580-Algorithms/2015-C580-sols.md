...
It seems that, the more bits there are, the more expensive it becomes (on average) to increment by one but the average cost given k seems to assymptotically approach a limit which means the procedure should be tightly bound by O(N). Not sure how to prove that though?
