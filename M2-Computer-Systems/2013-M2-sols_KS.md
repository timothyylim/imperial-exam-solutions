...
**(iii.)** TEMPORARY-UNCERTAIN ANSWER: In order to make sure the program execution resumes with the CPU state identical to taht immediately before the prcedure call, the assembly programmer should setup a frame poitner and allocate space for local variables upon entry and then de-allocate local variables and restore the frame pointer upon exit. [Slide 15](http://www.commsp.ee.ic.ac.uk/~kkleung/Computer_Systems_2015/7_Pentium4-5.ppt).
